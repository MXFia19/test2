...
function handleHome() {
    return new Response("Twitch Proxy Worker is Running!", { headers: COMMON_HEADERS });
}
...
async function handleGetM3U8(url) {
    ...
    // 2. Fallback Storyboard
    // (Simplifié pour le worker, car souvent moins nécessaire si le proxy marche bien)
...
async function handleProxy(url, request) {
    ...
    // Si c'est un segment vidéo (Proxying stream)
    const response = await fetch(targetUrl, { headers: COMMON_HEADERS });
...
function parseM3U8(content, masterUrl) {
    ...
    order.forEach(k => {
        Object.keys(unsorted).forEach(u => {
            if (u.toLowerCase().includes(k.toLowerCase())) {
                sorted[u] = unsorted[u];
                delete unsorted[u];
            }
        });
